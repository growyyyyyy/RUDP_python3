...
def tests_to_run(forwarder):
    ...
    from tests import OutOfOrderTest, SackOutOfOrderTest
...
class Forwarder(object):
    ...
    def _send(self, packet):
        """ Send a packet. """
        packet.update_packet(seqno=packet.seqno +
                             self.start_seqno_base, update_checksum=False)
    ...
    def handle_receive(self, message, address, sackMode=False):
        ...
        if self.test_state == "READY":
            if address == self.receiver_addr:
                p = Packet(message, self.sender_addr,
                           self.start_seqno_base, sackMode)
